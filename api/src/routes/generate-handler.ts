...
export const generateHandler: FastifyPluginAsyncZod = async (app) => {
  app.post(
    ...
    {
      schema: {
        summary: 'Genarate a TypeScript handler from a OpenAPI spec URL',
        tags: ['Webhooks'],
        body: z.object({
    ...
    async (request, reply) => {
      ...
      const result = await db.select({body: webhooks.body}).from(webhooks).where(inArray(webhooks.id, webhooksIds))
      const webhooksBodies = result.map(wh => wh.body).join('\n\n')
